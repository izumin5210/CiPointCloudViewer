...
void SensorDevice::setCalibrationParams(const Clouds::UpdateCalibrationParamsAction &action) {
...
void SensorDevice::updateFps(const FpsCounter::Event& event) {
...
void SensorDevice::startColorStream() {
...
void SensorDevice::startIrStream() {
...
void SensorDevice::enableMirroring() {
  if (!device_.isFile()) {
    if (color_stream_.isValid()) {
...
void SensorDevice::enableDepthToColorRegistration() {
...
void SensorDevice::updateColorImage(const openni::VideoFrameRef &color_frame) {
...
void SensorDevice::updateRawDepthImage(const openni::VideoFrameRef &depth_frame) {
  raw_depth_image_ = cv::Mat(depth_frame.getHeight(), depth_frame.getWidth(),
                             CV_16UC1,
                             (unsigned short *) depth_frame.getData());
}

void SensorDevice::updateDepthImage(const openni::VideoFrameRef &depth_frame) {
  cv::Mat depth_image;
...
void SensorDevice::updateIrImage(const openni::VideoFrameRef &ir_frame) {
  cv::Mat ir_image;
...
void SensorDevice::updatePointCloud(std::chrono::system_clock::time_point timestamp) {
